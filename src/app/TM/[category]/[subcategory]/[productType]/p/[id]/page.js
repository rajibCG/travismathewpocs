...
export default async function PLPPage({ params }) {
    ...
    try {
        ...
        const hybrisData = await fetch(
            `${process.env.HYBRIS_API_URL}/restv2/v2/b2c-us/products/${pid}?fields=FULL`, { next: { revalidate: 60 } }
        )
            .then((response) => response.json());
